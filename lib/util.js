...
exports.parsingPoiResult = function (poi_result, labels){
    var result = [];
    for (var i = 0; i < poi_result.length; i ++){
        var res = {};
        for (var j = 0; j < labels.length; j ++){
            res[labels[j]] = poi_result[i][j];
        }
        result.push(res);
    }
    return result;
...
exports.parsingBehaviorResult = function (behavior_result, labels){
    var result = {};
    for (var i = 0; i < behavior_result.length; i ++) {
        result[labels[i]] = behavior_result[i];
    }
    return result;
};